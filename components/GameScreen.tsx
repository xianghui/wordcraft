...
export const GameScreen: React.FC<GameScreenProps> = ({
  ...
  const [letterUsage, setLetterUsage] = useState<
    Record<string, { required: number; spelled: number }>
  >({});
  ...
  useEffect(() => {
    if (
      targetWord.length > 0 &&
      spelledWord.length === targetWord.length &&
      !isLevelComplete
    ) {
      if (!isAnswerRevealed) {
        playClappingSound();
  ...
  const handleBlockClick = useCallback(
    (block: BlockData) => {
      ...
      if (block.letter === nextLetterNeeded) {
        ...
        if (
          updatedUsage[block.letter].spelled ===
          updatedUsage[block.letter].required
        ) {
          setGrid((prevGrid) =>
            prevGrid.map((b) =>
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center p-0 sm:p-2 md:p-4 text-white">
      <HUD
